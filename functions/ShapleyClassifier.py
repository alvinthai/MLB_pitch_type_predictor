...
class ShapleyClassifier(BaseClassifier):
    '''
    ...
    def __init__(self, target=None, model=None, model_fit_params=None,
        ...
        init = super(ShapleyClassifier, self).__init__
    ...
    def _eval_set(self, eval_set):
        es = super(ShapleyClassifier, self)._eval_set
    ...
    def _xy_transform(self, X, y):
        xy_tf = super(ShapleyClassifier, self)._xy_transform
        X, y = xy_tf(X, y)
    ...
    def fit(self, X, y=None, eval_set=None):
        ...
        return super(ShapleyClassifier, self).fit(X, y, eval_set)

    def predict(self, X):
        X = self._input_cleanup(X)
        X = self._pred_cleanup(X)
        return super(ShapleyClassifier, self).predict(X)

    def predict_proba(self, X):
        X = self._input_cleanup(X)
        X = self._pred_cleanup(X)
        return super(ShapleyClassifier, self).predict_proba(X)
